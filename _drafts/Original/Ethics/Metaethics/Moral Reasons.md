...
Moral constraints:
- Everyone has reason to accept moral truth
- The ultimate truthmakers for moral truth must be the good/bad it imposes (though there can be deontic enablers/disablers).
- It is wrong for A to X only if it the badness that is imposed on an individual is not justified by the personal reasons that A has to perform X.
- Moral truths must be impartial
...
1. What are moral norms:
	- Attitude: Provide reasons for blame/shame/guilt.
	- Deliberative: Provide reasons that should figure in the reasoning of all rational agents.
	- Content: Provide reasons that are based on the interests of all agents.
...
Strategy
1. Moral reasons = reasons for blame/shame/guilt (aesthetic)
2. Moral reasons = reasons with intersubjective justifiability (constructed "institutional" norms in western civilization)
	- "Institutional" in the sense that these norms are not the upshots of reason/rationality (i.e. internal reasons). They are still normative because they are explained by reasons that are the upshots of rationality. I.e. moral institutional norms are explained by facts about reasons for action generally, which themselves are internal, i.e. the upshots of reason/rationality.
3. Reasons with intersubjective justifiability = reasons that can be reasonably accepted by all with the aim of finding such reasons. This seems indeterminate and the result of ongoing negotiation/cultural relativism. We need a procedure to determine how to reach conclusions. E.g. surely people have a duty to suffer 1 unit of suffering to alleviate 100 units. But people probably don't have a duty to suffer 20 units of suffering to alleviate 100 units. How much should people be committed to assisting others? This will likely be the result of the relationships that people willingly enter into?

Strategy for political philosophy
1. Empirical assumption. Assume humans have reason to desire certain primary goods, namely, property. Ascribe to people strong ownership rights to their body. 
2. Empirical assumption. Assume that there is very little agreement about the best way to live.
3. Given these, the only principles with intersubjective justifiability are those with minimal interference into the lives of others.
4. But how to determine what interferences count as significant. E.g. minor intrusions are permissible to prevent major intrusions. This seems to be an empirical question that cannot be determined a priori. Perhaps appeal to democratic vote? But what if a town decides to prevent people from, e.g., engaging in certain sexual practices within the privacy of their own home?
...
Against Railton's claim that morality = rationality from a social perspective
- This ignores the separateness of persons.
- How to aggregate? Just enumerate and add up? Is this intuitively plausible? Why not other forms of aggregation? Are these logistically plausible? E.g. that which we all have reason to accept
- Should we rule out certain interests, e.g. sadistic interests
- We should only be concerned with the personal reasons that people have, not their idealized desires (whether one's personal reasons are determined by their idealized desires is a further question) [this is what Railton says, but it's worth mentioning]
- Should we restrict ourselves to particular ends, e.g. the end of finding reasons that we all have reasons to accept
